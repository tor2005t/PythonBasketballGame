    ...
    def PlayGame(self):
        ...
        while (self.score1 or self.score2 <= 11):
            if (self.PlayerOnePossession == True):
                ShootAction = raw_input('What would you like to do ' + p1.name + '? Shoot a 3 pointer, Midrange shot or Layup? ').lower()
            if (self.PlayerTwoPossession == True):
                ShootAction = raw_input('What would you like to do ' + p2.name + '? Shoot a 3 pointer, Midrange shot or Layup? ').lower()
            if ((ShootAction == '3 pointer') or (ShootAction == 'midrange') or (ShootAction == 'layup')):
                if (ShootAction == '3 pointer'):
                    ...
                    if (self.PlayerOnePossession == True):
                        BlockAttempt = raw_input(p2.name + ', Would you like to try to block the shot or go rebound? ').lower()
                    else:
                        BlockAttempt = raw_input(p1.name + ', Would you like to try to block the shot or go rebound? ').lower()
